...
import { filterData, getUserToken } from '../../../utils/common';
...
export class RoleService {
  ...
  async findPermissions(
    findPermissionDto: FindPermissionDto,
  ): Promise<string[]> {
   ...
   * @param req
   */
  async update(role: UpdateRoleDto, _req: any): Promise<void> {
    ...
    if (permissions && permissions.length > 0) {
      ...
      roleEntity.permissions = await this.permissionRepository.find({
      ...
      // 如果没有权限字段，则直接更新
      const { permissions, ...updateData } = roleEntity;
